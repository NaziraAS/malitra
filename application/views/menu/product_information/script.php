<script>
	$.ajax({
		url: "<?= base_url("master_data/product_information/load") ?>",
		method: "GET",
		dataType: "JSON",
		success: function(data) {
			console.log(data)
			for (d of data) {
				$('#table-product-information').append(
					`<tr>
						<td class="text-left">${d.title}</td>
						<td class="text-center">
							<div>
								<div class="symbol symbol-50px">
								<img src="<?= base_url() ?>assets/images/product_information/${d.video}" class="" alt="" />
														</div>
													</div>
												</td>
												<td class="text-left">${(d.link ==null)?'Link tidak ada':d.link}</td>
												<td class="text-center">
		...
		if (id) {
			Swal.fire({
				...
				if (result.isConfirmed) {
					$.ajax({
						url: "<?= base_url("master_data/product_information/hapus") ?>",
						method: "POST",
						dataType: "JSON",
						data: {
							id
